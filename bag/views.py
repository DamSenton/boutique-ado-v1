...
def view_bag(request):
    """ A view to show a user their shopping bag """
    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['items_by_size'].keys():
                ...
                bag[item_id]['items_by_size'][size] = quantity
                messages.success(
                    request, f'Added size {size.upper()} {product.name} to your shopping bag!')
        else:
            bag[item_id] = {'items_by_size': {size: quantity}}
            messages.success(
                request, f'Added size {size.upper()} {product.name} to your shopping bag!')
...
def adjust_bag(request, item_id):
    """ A view to adjust the quantity of a specified product """
    ...
    if size:
        if quantity > 0:
            ...
            if not bag[item_id]['items_by_size']:
                bag.pop(item_id)
                messages.success(
                    request, f'Removed size {size.upper()} {product.name} from your shopping bag!')
    else:
        if quantity > 0:
            bag[item_id] = quantity
            messages.success(
